...
const TrendingScreentv = () => {
	...
	useEffect(() => {
		fetch(
			`https://api.themoviedb.org/3/tv/popular?api_key=9126021014a60e59b03c4b28f3264efb&page=${page}`
		)
			.then((data) => data.json())
			.then((json) => {
	...
	return (
		<SafeAreaView className='pt-6 px-5 flex-1'>
			<ImageBackground
				style={{
					width: "120%",
					height: "120%",
			...
			<ScrollView ref={scrollRef} className=' w-full'>
				<View className='flex-row flex-wrap justify-evenly'>
					{data?.map((item) => (
						<SingleTrendCard type='tv' key={item.id} item={item} />
					))}
				</View>
			</ScrollView>
...
export default TrendingScreentv
